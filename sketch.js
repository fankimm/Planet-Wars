  ...
  if (!isMobile) {
    ...
    if (!isGameOver && isIntroPlayed) {
      ...
      if (isItemUse) {
        ...
        rect(
          player.pos.x - player.rad,
          player.pos.y + 30,
          ((lastFrameCount - frameCount + 240) / 24) * 6.4,
          12
        );
    ...
    if (isGameOver & !isPost) {
      if (isInput) {
        isPost = true;
        var url =
          "https://script.google.com/macros/s/AKfycby0VLtnKlOsKmHFA-eiwptlgdW75PyY7ad7sTL4_wP9/dev" +
          "?name=" +
          name +
          "&score=" +
          score;
...
function playerMove(planet) {
  planet.vel.x =
    (mouseX - planet.pos.x) / dist(planet.pos.x, planet.pos.y, mouseX, mouseY);
  planet.vel.y =
    (mouseY - planet.pos.y) / dist(planet.pos.x, planet.pos.y, mouseX, mouseY);
}
